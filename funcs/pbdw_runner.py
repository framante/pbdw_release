...
class PBDWRunner:
    ...
    def _assemble_saddle_point_system(self):
        ...
        if self.permutation == "P":
            ...
            Fx = Qx @ self.SpaceMatrix.dot(self.R3dx.T)
            Fy = Qy @ self.SpaceMatrix.dot(self.R3dy.T)
            Fz = Qz @ self.SpaceMatrix.dot(self.R3dz.T)
            ...
            self.lhs[:self.Mx, :self.Mx] = Fx @ Fx.T
            self.lhs[self.Mx:self.Mx + self.My,
                     self.Mx:self.Mx + self.My] = Fy @ Fy.T
            self.lhs[self.Mx + self.My:self.M,
                     self.Mx + self.My:self.M] = Fz @ Fz.T
            self.lhs[:self.M, :self.M].flat[::self.M + 1] += self.M * self.xi

            # upper right: [ F_x F_x^\top G_x
            #                F_y F_y^\top G_y
            #                F_z F_z^\top G_z ]
            self.lhs[:self.Mx, self.M:] = Fx @ Fx.T @ Gx
            self.lhs[self.Mx:self.Mx + self.My, self.M:] = Fy @ Fy.T @ Gy
            self.lhs[self.Mx + self.My:self.M, self.M:] = Fz @ Fz.T @ Gz
    ...
    def _run_parallel(self, strategy):
        ...
        if self.permutation == "P":
            shm["Q"] = shmem.create_shared_array(self.Q)
            ...
            shm["isx"] = shmem.create_shared_array(self.isx)
            shm["isy"] = shmem.create_shared_array(self.isy)
            shm["isz"] = shmem.create_shared_array(self.isz)
        else:
    ...
    def _run_sequential(self, strategy):
        ...
        self.results = [
            seqmem.reconstruct_Aretz_seq(
                ...
                self.R3d, self.Q3d, self.V, self.Q, self.lhs,
                self.R3dx, self.R3dy, self.R3dz, self.isx, self.isy, self.isz)
    ...
    def _cleanup(self):
        ...
        # Large dense arrays
        for attr in ('W', 'R1d', 'SpaceMatrix', 'V', 'Q', 'lhs', 'R3dx', 'R3dy',
                     'R3dz', 'isx', 'isy', 'isz', 'quots', 'rests', 'chosen',
                     'invperm', 'perm', 'results', 'Ttmat', 'Pmat', 'R3d',
                     'Q3d', 'W3dinv'):
